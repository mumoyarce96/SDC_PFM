...
target_match_ids = previous_df_info[previous_df_info['n'] < 40]['match_id'].tolist()
target_matches = matches[matches['match_id'].isin(target_match_ids)]
print(len(target_matches))
df = get_player_stats(target_matches, previous_df, reruns = 4).drop_duplicates(subset=['match_id', 'player_id'], keep = 'last')
